...
form.addEventListener('submit', async (event) => {
  ...
  if (!file) {
    setStatus('error', 'אנא צרפו מסמך לעדכון המונה לפני השליחה.');
    return;
  }

  submitBtn.disabled = true;
  form.classList.add('is-loading');
  setStatus('info', 'מעלה את המסמך ושולח את בקשת עדכון המונה...');
  ...
  try {
    ...
    setStatus(
      'success',
      'בקשת עדכון המונה נשלחה בהצלחה! צוות חברת החשמל יחזור אליכם בקרוב.'
    );
    form.reset();
    setFile(null);
    showSuccessModal();
